...
const AgencyManager = () => {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Are you sure you want to delete this agency? This may affect records linked to it.",
      )
    ) {
      try {
        const res = await axios.delete(`/api/estimation/deleteAgency/${id}`, {
          withCredentials: true,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        res = await axios.put(
          `/api/estimation/updateAgency/${editingId}`,
          formData,
          {
            withCredentials: true,
          },
        ...
        res = await axios.post(`/api/estimation/createAgency`, formData, {
          withCredentials: true,
        });
      }
          ...
          <div className="bg-white w-full max-w-sm rounded-[40px] shadow-2xl overflow-hidden border border-white/20">
            ...
            <div
              className={`p-8 text-white flex justify-between items-center ${
                editingId ? "bg-amber-500" : "bg-slate-800"
              }`}
            >
              <div>
                <h2 className="text-xl font-black uppercase tracking-tight">
                  {editingId ? "Modify Agency" : "New Agency"}
                </h2>
