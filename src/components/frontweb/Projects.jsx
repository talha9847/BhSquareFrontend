...
const projects = [
  ...
  {
    id: 5,
    title: "Jaipur Farm Estate",
    category: "Agricultural",
    capacity: "75 kW",
    savings: "₹65,000/mo",
    image:
      "https://images.unsplash.com/photo-1521618755572-156ae0cdd74d?w=700&q=80",
    location: "Jaipur, Rajasthan",
    year: "2024",
  },
  {
    id: 6,
    title: "Mumbai Office Complex",
    category: "Commercial",
    capacity: "300 kW",
    savings: "₹2.5L/mo",
    image:
    ...
    <section id="projects" ref={ref} className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div
            className={`transition-all duration-700 transform ${hasInview ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
          ...
          <div
            className={`flex flex-wrap gap-2 transition-all duration-700 delay-100 transform ${hasInview ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 min-h-[400px]">
          {filtered.map((project, i) => {
            ...
            return (
              <div
                key={project.id}
                className={`group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 transform 
                  ${hasInview ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
                style={{ transitionDelay: hasInview ? `${i * 50}ms` : "0ms" }}
              >
                <div className="relative h-52 overflow-hidden">
                  ...
                  <span
                    className={`absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-semibold font-body ${colors.bg} ${colors.text}`}
                  >
                    {project.category}
                  </span>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-4 pt-4 border-t border-gray-100">
                    <div>
                      ...
                      <p className="font-display font-bold text-[#1a5695]">
                        {project.capacity}
                      </p>
                    </div>
        ...
        <div
          className={`text-center mt-12 transition-all duration-700 delay-300 transform ${hasInview ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
